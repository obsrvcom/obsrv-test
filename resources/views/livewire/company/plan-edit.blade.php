<div class="flex h-full w-full flex-1 flex-col gap-4">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-neutral-200 dark:border-neutral-700 p-6">
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-3">
                <flux:button size="sm" variant="ghost" href="{{ route('company.plans.category', ['company' => $company, 'category' => $planData->category]) }}">
                    <flux:icon name="arrow-left" class="size-4" />
                    Back to {{ $planData->category->name }} Plans
                </flux:button>
            ...
            <div class="flex gap-2">
                ...
                <flux:button size="sm" variant="ghost" href="{{ route('company.features.category', ['company' => $company, 'category' => $planData->category]) }}">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach($planData->revisions as $rev)
                            <div class="inline-flex items-center gap-1 px-3 py-2 rounded-lg text-sm font-medium transition-colors
                                ...
                                <a href="{{ route('company.plans.edit.revision', ['company' => $company, 'plan' => $planData, 'revision' => $rev]) }}"
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-neutral-200 dark:border-neutral-700 flex-1 overflow-hidden">
        <div class="h-full overflow-y-auto p-6">
            @include('livewire.company.plan-edit.grid', ['revisionData' => $revisionData, 'planData' => $planData])
        </div>
    </div>

    <!-- Modals -->
    @include('livewire.company.plan-edit.modals', ['editingRevisionData' => $revisionData])
</div>
