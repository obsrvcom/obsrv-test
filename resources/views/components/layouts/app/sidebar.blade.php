...
<flux:sidebar sticky stashable class="border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
    ...
    <flux:navlist variant="outline">
        <flux:navlist.group :heading="__('Company')" class="grid">
            ...
            <flux:navlist.item icon="building-office" :href="$currentCompany ? route('company.sites', ['company' => $currentCompany->id]) : '#'" :current="request()->routeIs('company.sites*')" wire:navigate>{{ __('Sites') }}</flux:navlist.item>
            <flux:navlist.item icon="document-text" :href="$currentCompany ? route('company.plans', ['company' => $currentCompany->id]) : '#'" :current="request()->routeIs('company.plans*')" wire:navigate>{{ __('Plans') }}</flux:navlist.item>
